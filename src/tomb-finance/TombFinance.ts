...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.yToken = new ERC20(deployments.yToken.address, provider, 'FXM');
    this.FTMX = new ERC20(deployments.ftmx.address, provider, 'FTMX');
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.yToken, this.FTMX, this.TBOND, ...Object.values(this.externalTokens)];
  ...
  async getYTokenStat(): Promise<TokenStat> {
    // const { SoulFtmRewardPool, TombFtmLpTombRewardPool, TombFtmLpTombRewardPoolOld } = this.contracts;
    const supply = await this.yToken.totalSupply();
    // const tombRewardPoolSupply = await this.yToken.balanceOf(SoulFtmRewardPool.address);
    // const tombRewardPoolSupply2 = await this.yToken.balanceOf(TombFtmLpTombRewardPool.address);
    // const tombRewardPoolSupplyOld = await this.yToken.balanceOf(TombFtmLpTombRewardPoolOld.address);
    const yTokenCirculatingSupply = supply;
    // TODO : NEED to substract all others sources for accurate circulating supply
    // .sub(tombRewardPoolSupply)
    // .sub(tombRewardPoolSupply2)
    // .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.yToken);
    const priceOfOneFTM = await this.getWFTMPriceFromPancakeswap();
    const priceOfYTokenInDollars = (Number(priceInFTM) * Number(priceOfOneFTM)).toFixed(2);

    return {
      tokenInFtm: priceInFTM,
      priceInDollars: priceOfYTokenInDollars,
      totalSupply: getDisplayBalance(supply, this.yToken.decimal, 0),
      circulatingSupply: getDisplayBalance(yTokenCirculatingSupply, this.yToken.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('TOMB') ? this.yToken : this.TSHARE;
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const tombStat = await this.getYTokenStat();
  ...
  async getShareStat(): Promise<TokenStat> {
    ...
    const supply = await this.FTMX.totalSupply();

    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.FTMX);
    const tombRewardPoolSupply = await this.FTMX.balanceOf(TombFtmLPTShareRewardPool.address);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.FTMX.decimal, 0),
      circulatingSupply: getDisplayBalance(tShareCirculatingSupply, this.FTMX.decimal, 0),
    };
  }

  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, SoulFtmRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.yToken.address, ethers.utils.parseEther('1'));
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.yToken.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.yToken.decimal, 0),
    };
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'SOUL' ? await this.getYTokenStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'SOUL') {
      if (!contractName.endsWith('SoulRewardPool')) {
    ...
    if (depositTokenName.startsWith('SOUL')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      if (tokenName === 'TOMB-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.yToken, true);
  ...
  async getLPTokenPrice(lpToken: ERC20, token: ERC20, isTomb: boolean): Promise<string> {
    ...
    const stat = isTomb === true ? await this.getYTokenStat() : await this.getShareStat();
  ...
  async getTokenPriceFromSpiritswap(tokenContract: ERC20): Promise<string> {
    ...
    const wftm = new TokenSpirit(chainId as any, WFTM.address, WFTM.decimal);
    const token = new TokenSpirit(chainId as any, tokenContract.address, tokenContract.decimal, tokenContract.symbol);
    try {
  ...
  async getMasonryAPR() {
    ...
    const TSHAREPrice = (await this.getShareStat()).priceInDollars;
    const TOMBPrice = (await this.getYTokenStat()).priceInDollars;
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === FTM_TICKER) {
      ...
      const token = tokenName === TOMB_TICKER ? this.yToken : this.TSHARE;
  ...
  async estimateAmountOfFTM(FTMAmount: string): Promise<string> {
    const { TShareSwapper } = this.contracts;
    try {
      const estimateBN = await TShareSwapper.estimateAmountOfTShare(parseUnits(FTMAmount, 18));
      return getDisplayBalance(estimateBN, 18, 6);
    } catch (err) {
      console.error(`Failed to fetch estimate tshare amount: ${err}`);
    }
